...
export interface FindManyRequest extends Request {
  prisma: PrismaClient;
  query: Prisma.FiatWalletFindManyArgs & ParsedQs;
...
export type FindManyMiddleware = RequestHandler<ParamsDictionary, any, any, Prisma.FiatWalletFindManyArgs & ParsedQs, Record<string, any>>

export async function FiatWalletFindMany(req: FindManyRequest, res: Response, next: NextFunction) {
  try {
    if (!req.outputValidation && !req.omitOutputValidation) {
      throw new Error('Output validation schema or omission flag must be provided.');
    }

    const data = await req.prisma.fiatWallet.findMany(req.query as Prisma.FiatWalletFindManyArgs);
    if (req.passToNext) {

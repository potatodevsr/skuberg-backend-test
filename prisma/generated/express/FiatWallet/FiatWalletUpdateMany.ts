...
interface UpdateManyRequest extends Request {
  prisma: PrismaClient;
  body: Prisma.FiatWalletUpdateManyArgs;
  outputValidation?: ZodTypeAny;
  omitOutputValidation?: boolean;
}

export type UpdateManyMiddleware = RequestHandler<ParamsDictionary, any, Prisma.FiatWalletUpdateManyArgs, Record<string, any>>

export async function FiatWalletUpdateMany(req: UpdateManyRequest, res: Response, next: NextFunction) {
  try {
    if (!req.outputValidation && !req.omitOutputValidation) {
      throw new Error('Output validation schema or omission flag must be provided.');
    }

    const data = await req.prisma.fiatWallet.updateMany(req.body);

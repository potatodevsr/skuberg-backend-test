...
interface GroupByRequest extends Request {
  prisma: PrismaClient;
  query: Partial<Prisma.CryptocurrencyGroupByArgs> & ParsedQs;
...
export async function CryptocurrencyGroupBy(req: GroupByRequest, res: Response, next: NextFunction) {
  try {
    ...
    // @ts-ignore
    const result = await req.prisma.cryptocurrency.groupBy(req.query);

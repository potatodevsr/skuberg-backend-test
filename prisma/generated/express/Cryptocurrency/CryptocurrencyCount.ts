...
interface CountRequest extends Request {
  prisma: PrismaClient;
  query: Partial<Prisma.CryptocurrencyCountArgs> & ParsedQs;
...
export async function CryptocurrencyCount(req: CountRequest, res: Response, next: NextFunction) {
  try {
    if (!req.outputValidation && !req.omitOutputValidation) {
      throw new Error('Output validation schema or omission flag must be provided.');
    }

    const result = await req.prisma.cryptocurrency.count(req.query as Prisma.CryptocurrencyCountArgs);

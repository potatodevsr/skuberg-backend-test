...
interface UpdateManyRequest extends Request {
  prisma: PrismaClient;
  body: Prisma.TransferUpdateManyArgs;
  outputValidation?: ZodTypeAny;
  omitOutputValidation?: boolean;
}

export type UpdateManyMiddleware = RequestHandler<ParamsDictionary, any, Prisma.TransferUpdateManyArgs, Record<string, any>>

export async function TransferUpdateMany(req: UpdateManyRequest, res: Response, next: NextFunction) {
  try {
    if (!req.outputValidation && !req.omitOutputValidation) {
      throw new Error('Output validation schema or omission flag must be provided.');
    }

    const data = await req.prisma.transfer.updateMany(req.body);

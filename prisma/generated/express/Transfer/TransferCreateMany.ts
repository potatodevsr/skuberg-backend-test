...
interface CreateManyRequest extends Request {
  prisma: PrismaClient;
  body: Prisma.TransferCreateManyArgs;
  outputValidation?: ZodTypeAny;
  omitOutputValidation?: boolean;
}

export type CreateManyMiddleware = RequestHandler<ParamsDictionary, any, Prisma.TransferCreateManyArgs, Record<string, any>>

export async function TransferCreateMany(req: CreateManyRequest, res: Response, next: NextFunction) {
  try {
    if (!req.outputValidation && !req.omitOutputValidation) {
      throw new Error('Output validation schema or omission flag must be provided.');
    }

    const data = await req.prisma.transfer.createMany(req.body);

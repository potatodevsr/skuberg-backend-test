...
export interface FindUniqueRequest extends Request {
  prisma: PrismaClient;
  query: Prisma.TransactionFindUniqueArgs & ParsedQs;
...
export type FindUniqueMiddleware = RequestHandler<ParamsDictionary, any, any, Prisma.TransactionFindUniqueArgs & ParsedQs, Record<string, any>>

export async function TransactionFindUnique(req: FindUniqueRequest, res: Response, next: NextFunction) {
  try {
    if (!req.outputValidation && !req.omitOutputValidation) {
      throw new Error('Output validation schema or omission flag must be provided.');
    }

    const data = await req.prisma.transaction.findUnique(req.query as Prisma.TransactionFindUniqueArgs);
    if (req.passToNext) {

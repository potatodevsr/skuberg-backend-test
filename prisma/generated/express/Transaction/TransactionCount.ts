...
interface CountRequest extends Request {
  prisma: PrismaClient;
  query: Partial<Prisma.TransactionCountArgs> & ParsedQs;
...
export async function TransactionCount(req: CountRequest, res: Response, next: NextFunction) {
  try {
    if (!req.outputValidation && !req.omitOutputValidation) {
      throw new Error('Output validation schema or omission flag must be provided.');
    }

    const result = await req.prisma.transaction.count(req.query as Prisma.TransactionCountArgs);

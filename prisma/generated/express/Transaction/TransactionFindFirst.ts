...
export interface FindFirstRequest extends Request {
  prisma: PrismaClient;
  query: Prisma.TransactionFindFirstArgs & ParsedQs;
...
export type FindFirstMiddleware = RequestHandler<ParamsDictionary, any, any, Prisma.TransactionFindFirstArgs & ParsedQs, Record<string, any>>

export async function TransactionFindFirst(req: FindFirstRequest, res: Response, next: NextFunction) {
  try {
    if (!req.outputValidation && !req.omitOutputValidation) {
      throw new Error('Output validation schema or omission flag must be provided.');
    }

    const data = await req.prisma.transaction.findFirst(req.query as Prisma.TransactionFindFirstArgs);
    if (req.passToNext) {

...
interface GroupByRequest extends Request {
  prisma: PrismaClient;
  query: Partial<Prisma.TransactionGroupByArgs> & ParsedQs;
...
export async function TransactionGroupBy(req: GroupByRequest, res: Response, next: NextFunction) {
  try {
    ...
    // @ts-ignore
    const result = await req.prisma.transaction.groupBy(req.query);

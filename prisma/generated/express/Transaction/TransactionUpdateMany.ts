...
interface UpdateManyRequest extends Request {
  prisma: PrismaClient;
  body: Prisma.TransactionUpdateManyArgs;
  outputValidation?: ZodTypeAny;
  omitOutputValidation?: boolean;
}

export type UpdateManyMiddleware = RequestHandler<ParamsDictionary, any, Prisma.TransactionUpdateManyArgs, Record<string, any>>

export async function TransactionUpdateMany(req: UpdateManyRequest, res: Response, next: NextFunction) {
  try {
    if (!req.outputValidation && !req.omitOutputValidation) {
      throw new Error('Output validation schema or omission flag must be provided.');
    }

    const data = await req.prisma.transaction.updateMany(req.body);

...
interface CountRequest extends Request {
  prisma: PrismaClient;
  query: Partial<Prisma.CryptoWalletCountArgs> & ParsedQs;
...
export async function CryptoWalletCount(req: CountRequest, res: Response, next: NextFunction) {
  try {
    if (!req.outputValidation && !req.omitOutputValidation) {
      throw new Error('Output validation schema or omission flag must be provided.');
    }

    const result = await req.prisma.cryptoWallet.count(req.query as Prisma.CryptoWalletCountArgs);

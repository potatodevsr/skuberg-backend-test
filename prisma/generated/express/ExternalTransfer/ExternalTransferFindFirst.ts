...
export interface FindFirstRequest extends Request {
  prisma: PrismaClient;
  query: Prisma.ExternalTransferFindFirstArgs & ParsedQs;
...
export type FindFirstMiddleware = RequestHandler<ParamsDictionary, any, any, Prisma.ExternalTransferFindFirstArgs & ParsedQs, Record<string, any>>

export async function ExternalTransferFindFirst(req: FindFirstRequest, res: Response, next: NextFunction) {
  try {
    if (!req.outputValidation && !req.omitOutputValidation) {
      throw new Error('Output validation schema or omission flag must be provided.');
    }

    const data = await req.prisma.externalTransfer.findFirst(req.query as Prisma.ExternalTransferFindFirstArgs);
    if (req.passToNext) {

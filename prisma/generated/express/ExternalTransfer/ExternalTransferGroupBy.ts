...
interface GroupByRequest extends Request {
  prisma: PrismaClient;
  query: Partial<Prisma.ExternalTransferGroupByArgs> & ParsedQs;
...
export async function ExternalTransferGroupBy(req: GroupByRequest, res: Response, next: NextFunction) {
  try {
    ...
    // @ts-ignore
    const result = await req.prisma.externalTransfer.groupBy(req.query);

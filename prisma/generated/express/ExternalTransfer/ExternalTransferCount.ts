...
interface CountRequest extends Request {
  prisma: PrismaClient;
  query: Partial<Prisma.ExternalTransferCountArgs> & ParsedQs;
...
export async function ExternalTransferCount(req: CountRequest, res: Response, next: NextFunction) {
  try {
    if (!req.outputValidation && !req.omitOutputValidation) {
      throw new Error('Output validation schema or omission flag must be provided.');
    }

    const result = await req.prisma.externalTransfer.count(req.query as Prisma.ExternalTransferCountArgs);

...
interface CountRequest extends Request {
  prisma: PrismaClient;
  query: Partial<Prisma.FiatCurrencyCountArgs> & ParsedQs;
...
export async function FiatCurrencyCount(req: CountRequest, res: Response, next: NextFunction) {
  try {
    if (!req.outputValidation && !req.omitOutputValidation) {
      throw new Error('Output validation schema or omission flag must be provided.');
    }

    const result = await req.prisma.fiatCurrency.count(req.query as Prisma.FiatCurrencyCountArgs);

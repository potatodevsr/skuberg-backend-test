...
export interface FindUniqueRequest extends Request {
  prisma: PrismaClient;
  query: Prisma.FiatCurrencyFindUniqueArgs & ParsedQs;
...
export type FindUniqueMiddleware = RequestHandler<ParamsDictionary, any, any, Prisma.FiatCurrencyFindUniqueArgs & ParsedQs, Record<string, any>>

export async function FiatCurrencyFindUnique(req: FindUniqueRequest, res: Response, next: NextFunction) {
  try {
    if (!req.outputValidation && !req.omitOutputValidation) {
      throw new Error('Output validation schema or omission flag must be provided.');
    }

    const data = await req.prisma.fiatCurrency.findUnique(req.query as Prisma.FiatCurrencyFindUniqueArgs);
    if (req.passToNext) {

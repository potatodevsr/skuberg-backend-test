...
export interface FindManyRequest extends Request {
  prisma: PrismaClient;
  query: Prisma.FiatCurrencyFindManyArgs & ParsedQs;
...
export type FindManyMiddleware = RequestHandler<ParamsDictionary, any, any, Prisma.FiatCurrencyFindManyArgs & ParsedQs, Record<string, any>>

export async function FiatCurrencyFindMany(req: FindManyRequest, res: Response, next: NextFunction) {
  try {
    if (!req.outputValidation && !req.omitOutputValidation) {
      throw new Error('Output validation schema or omission flag must be provided.');
    }

    const data = await req.prisma.fiatCurrency.findMany(req.query as Prisma.FiatCurrencyFindManyArgs);
    if (req.passToNext) {
